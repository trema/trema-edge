...
buffer *
pack_queue_multipart_request( VALUE options ) {
  ...
  uint32_t queue_id = OFPQ_ALL;
  VALUE r_queue_id = HASH_REF( options, queue_id );
  if ( !NIL_P( r_queue_id ) ) {
    queue_id = NUM2UINT( r_queue_id );
  }
  buffer *queue_multipart_request = create_queue_multipart_request( xid, flags, port_no, queue_id );
  return queue_multipart_request;
}
