...
static uint32_t
in_port_field( const bool attr, const enum oxm_ofb_match_fields oxm_type ) {
  uint32_t field = 0;

  if ( attr && oxm_type == oxm_in_port.type ) {
    field = OXM_OF_IN_PORT;
  }
  return field;
}
...
static uint16_t
in_port_length( const match *match ) {
  uint16_t length = 0;
  
  if ( match->in_port.valid ) {
    length = oxm_in_port.length;
  }
  return length;
}
