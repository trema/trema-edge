...
PaperHouse::RubyExtensionTask.new :libruby do | task |
  ...
  task.library_dependencies = %w(trema pthread rt dl crypt m)
end
...
PaperHouse::ExecutableTask.new :switch_manager do | task |
  ...
  task.library_dependencies = %w(trema pthread rt dl)
end
...
PaperHouse::ExecutableTask.new :switch_daemon do | task |
  ...
  task.cflags = CFLAGS
  task.ldflags = "-L#{ Trema.lib }"
  task.library_dependencies = %w(trema pthread rt dl ssl crypto)
end
...
PaperHouse::ExecutableTask.new :trema_switch do | task |
  ...
  task.library_dependencies = %w(ofdp trema pthread rt dl)
end
...
Rake::Task[:tremashark].prerequisites.each do | utility |
  begin
    ...
    PaperHouse::ExecutableTask.new utility do | task |
      ...
      task.cflags = CFLAGS
      task.ldflags = "-L#{ Trema.lib }"
      task.library_dependencies = %w(trema pthread rt dl pcap)
    end
  end
end
...
switch_tests.keys.each do | each |
  ...
  PaperHouse::ExecutableTask.new each do | task |
    ...
    task.library_dependencies = %w(cmockery ofdp trema pthread rt dl)
  end
end
